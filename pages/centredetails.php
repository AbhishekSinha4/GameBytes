...
<?php 
		...
		";
		$query="select * from center_manager where l_id='".$_GET["l_id"]."';";
		$result=mysqli_query($conn,$query);
		echo "<p>Manager: ";
		...
		echo"
			...
			<div class='clearing'></>";
		if(!IsSet($_GET["g_id"])){
			echo "<div id='games'><h2>All Games</h2>";

			$query="select * from game natural join game_location where l_id='".$_GET["l_id"]."';";
			$result=mysqli_query($conn,$query);
			for($j=0;$j<$result->num_rows;$j++){
				$row=mysqli_fetch_assoc($result);
				echo
		...
		$query="select * from location_reviews natural join member where l_id='".$_GET["l_id"]."';";
		$result=mysqli_query($conn,$query);
		for($j=0;$j<$result->num_rows;$j++){
			$row=mysqli_fetch_assoc($result);
			echo
			"
				...
				<div class='eachgame'>
					...
					<div class='details'>
						<p>Title: ".$row['l_r_title']."</p>
						<p>Review: ".$row['l_r_review']."</p>
						<p>Stars: ".$row['l_r_stars']."/10</p>
					</div>
					<div class='clearing'></div>
				</div>
			";
