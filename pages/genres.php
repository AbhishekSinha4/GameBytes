...
<?php 
		...
		for($i=0;$i<$copy->num_rows;$i++){
			...
			echo"<div class='tag'><a href='#".$row["genre_name"]."'>".$row["genre_name"]."</a></div>";
		}
		for($i=0;$i<$result->num_rows;$i++){
			$row=mysqli_fetch_assoc($result);
			echo "<div class='genre' id='".$row["genre_name"]."'><h2>".$row["genre_name"]."</h2>";
			//echo "select * from game natural join game_genre where genre_id=".$row["genre_id"]." order by g_name;";
			$query2="select * from game natural join game_genre where genre_id='".$row["genre_id"]."' order by g_name;";
